<template>
    <el-dialog :title="!dataForm.categoryId ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
            ...
            <el-form-item label="所属分类" prop="type">
                <el-select placeholder="请选择所属分类" clearable filterable v-model="dataForm.type"
                    ...
                    <el-option v-for="type in typeList" :key="type.parKey" :label="type.parValue" :value="type.parKey">
                    </el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="上级目录" prop="parentId">
                <el-popover ref="categoryListPopover" placement="bottom-start" trigger="click">
                    <el-tree :data="categoryList" :props="categoryListTreeProps" node-key="id" ref="categoryListTree"
                        @current-change="categoryListTreeCurrentChangeHandle" :default-expand-all="true"
...
export default {
    ...
    methods: {
        init(id) {
            ...
            if (this.dataForm.id) {
                this.$http({
                    url: `/admin/operation/category/info?id=${this.dataForm.id}`,
                    method: 'get',
                    ...
                    this.$http({
                        url: '/admin/operation/category/select',
                        method: 'get',
        ...
        dataFormSubmit() {
            this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                    this.$http({
                        url: `/admin/operation/category/${!this.dataForm.id ? 'save' : 'update'}`,
                        method: 'post',
